...
public class A_JSONMessage {
	...
	public static TCResult unpackStatusJson(JavaDict dict,JavaList list,String serviceIdentifier, String cacheName,String pattern) {
		...
		*/
		List keyList = CacheOperationImpl.keys(serviceIdentifier,
				cacheName, pattern);
		...
		for (int i = 0; i < keyList.size(); i++) {
			...
			//lk 20180909 修改 匹配一个SP多个名字的情况 如：密码键盘 不同的厂商命名规则不同，就会产生不同的名字：Encryptor、PinKeypad。解决方案：设备模块表的：MODSTATPATH由一个变为多个：Encryptor.status.device#PinKeypad.status.device
			if(map_value.indexOf("#")!=-1){
				//一个模块包含多个SPname
				for(int j=0;j<map_value.split("#").length;j++){
					String spPath = map_value.split("#")[j].toString();
					if (spPath.indexOf(".")!=-1){
						AppLogger.info("map_valueSplit:"+spPath.split("\\.").toString());
						mapDict.setItem(spPath.split("\\.")[0], spPath);
					}
				}
			}else{
				if (map_value.indexOf(".")!=-1){
