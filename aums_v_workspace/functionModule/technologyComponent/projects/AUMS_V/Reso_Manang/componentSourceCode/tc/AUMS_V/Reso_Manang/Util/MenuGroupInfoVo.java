package tc.AUMS_V.Reso_Manang.Util;
import javax.persistence.Entity;
import javax.persistence.Table;
import javax.persistence.Column;
import javax.persistence.Id;
/** 
 * 
 * @author  author
 */
@Entity() @Table(name="AUMS_MENU_MENUGROUPINFO") public class MenuGroupInfoVo {
  private String THEMECOLOR;
  private String LAYOUTROW;
  private String LAYOUTCOL;
  private String REMARK1;
  private String REMARK3;
  private String CATEGORYNAME;
  private String REMARK2;
  private String GROUP_ID;
  
  /** 
 * 样式列
 */
  @Column(name="LAYOUTCOL",nullable=true,unique=false,insertable=true,updatable=true)public String getLAYOUTCOL() {
	return LAYOUTCOL;
  }
  public void setLAYOUTCOL(String lAYOUTCOL) {
	 LAYOUTCOL = lAYOUTCOL;
  }
  
  public MenuGroupInfoVo(){
  }
  public void setTHEMECOLOR(  String THEMECOLOR){
    this.THEMECOLOR=THEMECOLOR;
  }
  /** 
 * 菜单组header背景颜色
 */
  @Column(name="THEMECOLOR",nullable=true,unique=false,insertable=true,updatable=true) public String getTHEMECOLOR(){
    return this.THEMECOLOR;
  }
  public void setLAYOUTROW(  String LAYOUTROW){
    this.LAYOUTROW=LAYOUTROW;
  }
  /** 
 * 样式行
 */
  @Column(name="LAYOUTROW",nullable=true,unique=false,insertable=true,updatable=true) public String getLAYOUTROW(){
    return this.LAYOUTROW;
  }
  public void setREMARK1(  String REMARK1){
    this.REMARK1=REMARK1;
  }
  /** 
 * 备注1
 */
  @Column(name="REMARK1",nullable=true,unique=false,insertable=true,updatable=true) public String getREMARK1(){
    return this.REMARK1;
  }
  public void setREMARK3(  String REMARK3){
    this.REMARK3=REMARK3;
  }
  /** 
 * 备注3
 */
  @Column(name="REMARK3",nullable=true,unique=false,insertable=true,updatable=true) public String getREMARK3(){
    return this.REMARK3;
  }
  public void setCATEGORYNAME(  String CATEGORYNAME){
    this.CATEGORYNAME=CATEGORYNAME;
  }
  /** 
 * 菜单组header内标题
 */
  @Column(name="CATEGORYNAME",nullable=true,unique=false,insertable=true,updatable=true) public String getCATEGORYNAME(){
    return this.CATEGORYNAME;
  }
  public void setREMARK2(  String REMARK2){
    this.REMARK2=REMARK2;
  }
  /** 
 * 备注2
 */
  @Column(name="REMARK2",nullable=true,unique=false,insertable=true,updatable=true) public String getREMARK2(){
    return this.REMARK2;
  }
  public void setGROUP_ID(  String GROUP_ID){
    this.GROUP_ID=GROUP_ID;
  }
  /** 
 * 组ID,对应表t_pcva_client_menugroupinfo的组id
 */
  @Id() @Column(name="GROUP_ID",nullable=true,unique=false,insertable=true,updatable=true) public String getGROUP_ID(){
    return this.GROUP_ID;
  }
}
