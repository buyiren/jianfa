...
public class P_Json {
	 ...
	 *         1 成功<br/>
	 */
	@InParams(param = { @Param(name = "jsonStr", comment = "json字符串", type = java.lang.String.class) })
	@OutParams(param = { @Param(name = "tarList", comment = "转换后的JavaList", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class) })
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "json串转javaList", style = "判断型", type = "同步组件", comment = "将入参的json字符串转换为JavaList对象,入参字符串必须符合jsonArray规范", author = "Anonymous", date = "2016-09-19 10:07:03")
	public static TCResult P_strToList(String jsonStr) {
		if (jsonStr == null || jsonStr.equals("")) {
			return TCResult.newFailureResult(ErrorCode.AGR,
					"jsonStr参数为null或为空字符串");
		}
		JSONArray jsonObj = (JSONArray) JSON.parseArray(jsonStr);
		JavaList jsonList=getJavaList(jsonObj);
		return TCResult.newSuccessResult(jsonList);
	}
