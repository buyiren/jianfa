...
public class P_Dict {
	...
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "容器变量删除", style = "判断型", type = "同步组件", comment = "删除容器中指定的变量", date = "Wed Jul 01 17:15:17 CST 2015")
	...
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "删除容器中空字段", style = "判断型", type = "同步组件", comment = "删除容器中空字段,空的标准为空字符串或null", date = "Wed Jul 01 17:27:45 CST 2015")
	 ...
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaDict}
	 * @param strOrList
	 *            入参|[['源key名称','目的key名称'],...]或字符串,如果strOrList为String类型，而不是列表时 则
	...
	@InParams(param = {
			...
			@Param(name = "strOrList", comment = "[['源key名称','目的key名称'],...]或字符串,如果strOrList为String类型，而不是列表时， 则进行匹配拷贝", type = java.lang.Object.class) })
	@OutParams(param = {})
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "容器变量拷贝", style = "判断型", type = "同步组件", comment = "拷贝映射源容器字典中的变量到目的容器中", date = "Wed Jul 01 17:53:45 CST 2015")
	 ...
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaDict}
	 * @param paramList
	 *            入参|检查容器中变量的列表,如：[[key1,类型(,长度)],[key1,类型(,长度)]...]|
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaList}
	 * @return 0 失败<br/>
	 *         1 判断变量正确<br/>
	 *         2 判断变量错误<br/>
	...
	@InParams(param = {
			...
			@Param(name = "paramList", comment = "检查容器中变量的列表,如：[[key1,类型(,长度)],[key1,类型(,长度)]...]", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class) })
	@OutParams(param = {})
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "判断变量正确"),
			@Return(id = "2", desp = "判断变量错误") })
	@Component(label = "容器变量检查", style = "选择型", type = "同步组件", comment = "检查字典中key对应value的类型和长度", date = "Thu Jul 02 10:00:27 CST 2015")
	...
	@InParams(param = {
			@Param(name = "inContext", comment = "请求容器", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class),
			@Param(name = "paramList", comment = "转换变量列表,如：[['源容器key名称',int/float/String类型,value(可没有)],...]", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class) })
	@OutParams(param = {})
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "容器变量转换", style = "判断型", type = "同步组件", comment = "容器中key对应的value变量转换为指定的类型和值value", date = "Thu Jul 02 10:39:46 CST 2015")
	...
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "容器变量赋值", style = "判断型", type = "同步组件", comment = "容器变量赋值", date = "Thu Jul 02 11:48:56 CST 2015")
			...
			if (!(data instanceof JavaList)) {
				if (!(inContext.getItem(data) instanceof JavaList)) {
					...
					if(!(obj instanceof JavaList)) {
						return TCResult.newFailureResult(ErrorCode.AGR, "源容器中键所对应的值必须为列表类型");
					}
					outContext.setItem(listData.getItem(1),((JavaList) obj).getItem(loopVar));
					break;
				}
			} else {
				if (!(data instanceof JavaList)) {
					return TCResult.newFailureResult(ErrorCode.AGR, "源容器到目的容器的映射列表中的子项必须为列表类型");
				}
	 ...
	 */
	@InParams(param = {
			@Param(name = "keyList", comment = "容器的键列表", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class),
			@Param(name = "records", comment = "容器的数据", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class),
			@Param(name = "context", comment = "容器", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class) })
	@OutParams(param = {})
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "多笔记录列赋值", style = "判断型", type = "同步组件", comment = "多笔记录列赋值,keyList的长度要和records中的记录长度一致", date = "Thu Jul 02 14:44:36 CST 2015")
	public static TCResult setColumeValue(JavaList keyList, JavaList records,
			JavaDict context) {
		if (keyList == null) {
			return TCResult.newFailureResult(ErrorCode.AGR, "入参容器的键集合不能为空");
		}
		if (records == null) {
			return TCResult.newFailureResult(ErrorCode.AGR, "入参容器的数据不能为空");
		}
		...
		while (it.hasNext()) {
			Object obj = it.next();
			if(!(obj instanceof JavaList)) {
				return TCResult.newFailureResult(ErrorCode.AGR, "入参容器的数据的子项必须为列表类型");
			}
			...
			for (int i = 0; i < keyLen; i++) {
				...
				if(!(context.getItem(keyList.getItem(i)) instanceof JavaList)) {
					return TCResult.newFailureResult(ErrorCode.AGR, "容器中键为" + keyList.getItem(i) + "对应的值必须为列表类型");
				}
	...
	public static TCResult covList(JavaDict context, JavaList listInfo) {
		...
		for (int i = 0; i < listInfo.size(); i++) {
			...
			if (!(colInfo.getItem(1) instanceof JavaList)) {
				return TCResult.newFailureResult(ErrorCode.AGR, "转换信息列表第" + (i + 1) + "项的第1个元素必须为列表类型");
			}
			...
			if (!(colInfo.getItem(2) instanceof JavaList)) {
				return TCResult.newFailureResult(ErrorCode.AGR, "转换信息列表第" + (i + 1) + "项的第2个元素必须为列表类型");
			}
	...
	public static TCResult multiCovList(JavaDict context, JavaList listInfo) {
		...
		if (!(context.getItem(listInfo_0.getItem(0)) instanceof JavaList)) {
			return TCResult.newFailureResult(ErrorCode.AGR, "字典容器中键为" + listInfo_0.getItem(0) + "对应的值必须为列表类型");
		}
		...
		for (int i = 0; i < tmpLen; i++) {
			for (int j = 0; j < listInfo.size(); j++) {
				...
				if (!(colInfo.getItem(1) instanceof JavaList)) {
					return TCResult.newFailureResult(ErrorCode.AGR, "转换信息列表第" + (j + 1) + "个子项下的第1个元素必须为列表类型");
				}
				if (!(colInfo.getItem(2) instanceof JavaList)) {
					return TCResult.newFailureResult(ErrorCode.AGR, "转换信息列表第" + (j + 1) + "个子项下的第2个元素必须为列表类型");
				}
				if (!(context.getItem(colInfo.getItem(0)) instanceof JavaList)) {
					return TCResult.newFailureResult(ErrorCode.AGR, "字典容器中键为" + colInfo.getItem(0) + "对应的值必须为列表类型");
				}
	 ...
	 *            入参|行号|int
	 * @param tarKeyList
	 *            入参|目标变量列表(如果为null，则和源列表相同)|
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaList}
	 * @return 0 失败<br/>
	 *         1 成功<br/>
	 */
	@InParams(param = {
			@Param(name = "__SRC__", comment = "数据源容器", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class),
			...
			@Param(name = "line", comment = "行号", type = int.class),
			@Param(name = "tarKeyList", comment = "目标变量列表(如果为null，则和源列表相同)", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class) })
	@OutParams(param = {})
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "容器变量多列值获取", style = "判断型", type = "同步组件", comment = "根据容器中的key在相应的值列表中获取相应的值", date = "Thu Jul 02 17:55:18 CST 2015")
	public static TCResult getKeyList(JavaDict __SRC__, JavaDict __TAR__,
		...
		if(!(__SRC__.getItem("keys") instanceof JavaList)) {
			return TCResult.newFailureResult(ErrorCode.AGR, "源容器中键为keys的值必须为列表类型");
		}
		if(!(__SRC__.getItem("values") instanceof JavaList)) {
			return TCResult.newFailureResult(ErrorCode.AGR, "源容器中键为values的值必须为列表类型");
		}
		// 获取key列表
		...
		for (int i = 0; i < srcKeyList.size(); i++) {
			...
			if (!(valueList.getItem(line-1) instanceof JavaList)) {
				return TCResult.newFailureResult(ErrorCode.AGR, "源容器中键为values对应列表值的第" + line + "个元素必须为列表类型");
			}
			// 获取对应的值
	 ...
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaList}
	 * @param dict1
	 *            入参|比较容器,如：{'key1':'1','key2':2....}|
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaDict}
	 * @param dict2
	 *            入参|被比较容器,如：{'key1':'1','key2':2....}|
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaDict}
	 * @return 0 判断变量错误<br/>
	 *         1 判断变量正确<br/>
	 */
	@InParams(param = {
			...
			@Param(name = "dict1", comment = "比较容器,如：{'key1':'1','key2':2....}", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class),
			@Param(name = "dict2", comment = "被比较容器,如：{'key1':'1','key2':2....}", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class) })
	@OutParams(param = {})
	...
	private static TCResult logForValidKeyValue (Object obj1, Object obj2, Object key) {
		AppLogger.trace(String.valueOf(obj1) + ":" + String.valueOf(obj2));
		return TCResult.newFailureResult(ErrorCode.AGR, "键：" + String.valueOf(key)
				+ "值不一致");
	}
	...
	@InParams(param = {
			...
			@Param(name = "cycleTime", comment = "当前循环次数", type = int.class),
			@Param(name = "dctDict", comment = "目的数据容器(合集)", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class) })
	@OutParams(param = {})
	@Returns(returns = { @Return(id = "0", desp = "判断变量错误"),
			@Return(id = "1", desp = "判断变量正确") })
	...
	public static TCResult multDictUnite(JavaDict srcDict, int cycleTime,
		...
		while (it1.hasNext()) {
			...
			if (!(obj instanceof JavaList)) {
				return TCResult.newFailureResult(ErrorCode.AGR, "目的数据容器中键为" + key + "对应的值必须为列表类型");
			}
		...
		while (it2.hasNext()) {
			...
			if (!srcDict.hasKey(key)) {
				if (!(dctDict.getItem(key) instanceof JavaList)) {
					return TCResult.newFailureResult(ErrorCode.AGR, "目的数据容器的子项必须为列表类型");
				}
