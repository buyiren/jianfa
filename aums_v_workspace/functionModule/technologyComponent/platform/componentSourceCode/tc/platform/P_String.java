...
public class P_String {
	...
	public static TCResult joinStr(JavaList strList, String sep) {
		...
		AppLogger.info("strlist: "+strList.toString()+"listsize(): "+strList.size());
	...
	@OutParams(param = { @Param(name = "asciiinfo", comment = "目标容器", type = cn.com.agree.afa.svc.javaengine.context.JavaDict.class) })
	@Returns(returns = { @Return(id = "0", desp = "处理失败"),
			@Return(id = "1", desp = "处理成功") })
	@Component(label = "字符转码ASCII2EBCD", style = "判断型", type = "同步组件", comment = "将输入的字典容器中所有key的值字符转码ASCII2EBCD", author = "姚园-3958", date = "2016-01-04 09:50:16")
	public static TCResult P_Str_ASCII2EBCD(JavaDict data_context) {
		if(!(data_context instanceof JavaDict)){
			return TCResult.newFailureResult("FBAC01","入参[数据容器]应为[dict]类型");
		}
		if(data_context.size()==0){
			return TCResult.newFailureResult("FBE999", "入参[数据容器]为空");
		}
		JavaDict asciiinfo=new JavaDict();
		Set<?> keySet = data_context.keySet();
		Object[] keylist = keySet.toArray();
		Object onevalue = data_context.get(keylist[0]);
		Object oldtmpvalue=null;
		Object newtmpvalue=null;
		byte[] oldtmpvalueBytes=null;
		for(Object key:keylist){
			if(onevalue instanceof String){
				oldtmpvalue = data_context.get(key);
				oldtmpvalueBytes=((String) oldtmpvalue).getBytes();
				newtmpvalue=AsciiAndEbcd.ASCIIToEBCDIC(oldtmpvalueBytes);
			    asciiinfo.put(key,newtmpvalue);
			}else if(onevalue instanceof JavaList){
				asciiinfo.put(key,new JavaList());
				for(int value=0;value<((JavaList) onevalue).size();value++){
					oldtmpvalue=((JavaDict) data_context.get(key)).get(value);
					oldtmpvalueBytes=((String) oldtmpvalue).getBytes();
					newtmpvalue=AsciiAndEbcd.ASCIIToEBCDIC(oldtmpvalueBytes);
					asciiinfo.put(key,((JavaList) asciiinfo.get(key)).add(newtmpvalue));
				}
			}
		}
		return TCResult.newSuccessResult(asciiinfo);
	}

	/**
	 * @category 字符转码EBCD2ASCII
	 * @param data_context
	 *            入参|数据容器|
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaDict}
	 * @param ver
	...
	@Returns(returns = { @Return(id = "0", desp = "处理失败"),
			@Return(id = "1", desp = "处理成功") })
	@Component(label = "字符转码EBCD2ASCII", style = "判断型", type = "同步组件", comment = "将输入的字典容器中所有key的值字符转码EBCD2ASCII", author = "姚园-3958", date = "2016-01-04 09:50:16")
	public static TCResult P_Str_EBCD2ASCII(JavaDict data_context, String ver,
			String area) {
		if(!(data_context instanceof JavaDict)){
			return TCResult.newFailureResult("FBAC01","入参[数据容器]应为[dict]类型");
		}
		...
		for(Object key:keylist){
			if(onevalue instanceof String){
				oldtmpvalue = data_context.get(key);
				oldtmpvalueBytes=((String) oldtmpvalue).getBytes();
				newtmpvalue=AsciiAndEbcd.EBCDICToASCII(oldtmpvalueBytes);
			    asciiinfo.put(key,newtmpvalue);
			}else if(onevalue instanceof JavaList){
				asciiinfo.put(key,new JavaList());
				for(int value=0;value<((JavaList) onevalue).size();value++){
	 ...
	 *            入参|分割标识字符|{@link java.lang.String}
	 * @param sep
	 *            入参|分割次数(大于0标识分割次数,等于0表示尽可能的分割且忽略末尾空字符串,小于0表示尽可能的分割且不忽略末尾空字符串)|{@link jint.String}	 *            
	 * @param outStrs
	 *            出参|拆分后的字符串list|
	 *            {@link cn.com.agree.afa.svc.javaengine.context.JavaList}
	 * @return 0 失败<br/>
	...
	@InParams(param = {
			@Param(name = "inStr", comment = "输入字符串", type = java.lang.String.class),
			@Param(name = "sep", comment = "分割标识字符", type = java.lang.String.class),
			@Param(name = "k", comment = "分割次数(大于0标识分割次数,等于0表示尽可能的分割且忽略末尾空字符串,小于0表示尽可能的分割且不忽略末尾空字符串)", type = int.class)})
	@OutParams(param = { @Param(name = "outStrs", comment = "拆分后的字符串list", type = cn.com.agree.afa.svc.javaengine.context.JavaList.class) })
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "标准字符串分割", style = "判断型", type = "同步组件", comment = "分割字符串", date = "Fri Jun 15 11:42:48 CST 2017")	
	public static TCResult StringSplit(String inStr, String sep,int k) {
		if (inStr == null) {
			return TCResult.newFailureResult(ErrorCode.AGR, "输入字符串参数非法，为空");
		}
		...
		JavaList resultList = new JavaList();
		String[] strArrs = inStr.split(sep,k);
		for (int i = 0; i < strArrs.length; i++) {
			resultList.add(strArrs[i]);
		}
		return TCResult.newSuccessResult(resultList);
